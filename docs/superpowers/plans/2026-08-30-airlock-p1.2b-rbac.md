...
`ServerDeps` 下面引用了 `*GrantAPI`，但那个类型要到 Task 15 才有实现。在改 `server.go` 之前，先建一个空壳类型让本任务能编译：

创建 `internal/control/grantapi.go`：

```go
package control

// GrantAPI 提供角色、授予与成员归属的 HTTP 接口。
// 这里先占位让 ServerDeps 能编译；完整实现在 Task 15。
type GrantAPI struct{}
```

再把 `internal/control/server.go` 整体替换为：
...
**Files:**
- Modify: `internal/control/grantapi.go`（替换 Task 11 里为让 `ServerDeps` 能编译而先放的 `GrantAPI{}` 空占位）
