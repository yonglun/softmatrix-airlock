...
> 拦截的，绝对不会超支。中间层级（比如"平台产品部""网关组"）的预算和用量，我们会实时汇总子团队数据展
> 示给您，方便您做预算规划和月度对账，但目前版本这些中间层的限额是"软限制"——不会在请求发生的瞬间被
> 硬性拦截，而是我们会在超支后第一时间告警。如果您需要中间层也做到硬性实时拦截，这是我们下一阶段
> （P2）自建配额引擎要解决的问题，我们可以把您这个诉求纳入优先级评估。"

要点：不夸大（不能说"五层都能强制执行"），不掩盖缺口（明确说中间层是软限制/告警），给出时间线预期
（P2 才能补上）。
